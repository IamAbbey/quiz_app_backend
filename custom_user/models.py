...
class EmailRecord(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        """
        user_email_list = list(User.objects.filter(is_active=True, is_staff=False, is_superuser=False).values_list('email', flat=True))
        self.no_of_user = len(user_email_list)
        custom_send_mail(subject=self.subject, message=self.message, list_of_email=user_email_list)
